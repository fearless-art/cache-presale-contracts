...
describe("PresaleRegister", () => {
    ...
    async function deployContracts() {
        ...
        const presale = (await hre.viem.deployContract("PresaleRegister", [
    ...
    describe("Deployment", () => {
        ...
        it("should have the correct hard cap", async () => {
            ...
            expect(hardCap).to.equal(parseEther("25000000")); // 25 million tokens
        });
    });

    describe("Buy Tokens With ERC20 Tokens", () => {
        it("should allow buying tokens with USDC", async () => {
            const { usdc, presale } = await deployContracts();

            // Prepare parameters for buyTokens
            const paymentToken = usdc.address;
            const amountPaid = parseUnits("100", 6); // 100 USDC
            const amountReceived = parseEther("1000"); // 1000 tokens
            const timeSigned = BigInt(currentTimestamp + 60); // 1 minute in the future
            const salt = 123n;

            // Create domain for EIP-712 signature
            const domain = {
                name: "PresaleRegister",
                version: "1",
            ...
            // Use the tokens to buy tokens
            await presale.write.buyTokens([
                paymentToken,
                amountPaid,
                amountReceived,
                Number(timeSigned),
            ...
            const userTokens = await presale.read.getUserTotalTokens([user.account?.address!]);
            expect(userTokens).to.equal(amountReceived);
        ...
        it("should fail if signature is already used", async () => {
            ...
            await expect(presale.write.buyTokens([
                paymentToken,
                amountPaid,
                amountReceived,
                Number(timeSigned),
        ...
        it("should fail if signature is expired", async () => {
            ...
            // Prepare parameters for buyTokens with expired timestamp
            const paymentToken = usdc.address;
            const amountPaid = parseUnits("100", 6); // 100 USDC
            const amountReceived = parseEther("1000"); // 1000 tokens
        ...
        it("should fail if token sale hard cap is reached", async () => {
            ...
            // Get the hard cap
            const hardCap = await presale.read.tokenSaleHardCap() as bigint;

            // Prepare parameters for buyTokens with amount that would exceed hard cap
            const paymentToken = usdc.address;
    ...
    describe("Buy Tokens With ETH", () => {
        it("should allow buying tokens with ETH", async () => {
            const { presale } = await deployContracts();

            // Prepare parameters for buyTokensWithEth
            const amountPaid = parseEther("1"); // 1 ETH
            const amountReceived = parseEther("5000"); // 5000 tokens
            const timeSigned = BigInt(currentTimestamp + 60); // 1 minute in the future
            const salt = 456n;
            ...
            const signature = await admin.signTypedData({
                ...
                domain,
                types,
                primaryType: "BuyTokensWithEthMessage",
                message,
            });

            // Use ETH to buy tokens
            await presale.write.buyTokensWithEth([
                amountPaid,
                amountReceived,
                Number(timeSigned),
                salt,
        ...
        it("should fail if ETH amount does not match signed amount", async () => {
            ...
            await expect(presale.write.buyTokensWithEth([
                amountPaid,
                amountReceived,
                Number(timeSigned),
                salt,
        ...
        it("should reject direct ETH transfers", async () => {
            ...
            ).to.be.rejectedWith("Use buyTokensWithEth instead");
        });
    });
    ...
    describe("Admin Functions", () => {
        ...
        it("should use Ownable2Step for ownership transfer", async () => {
            ...
            let currentOwner = await presale.read.owner() as string;
            ...
            currentOwner = await presale.read.owner() as string;
    ...
    describe("View Functions", () => {
        it("should correctly retrieve user purchase history", async () => {
            ...
            for (let i = 0; i < 2; i++) {
                ...
                const amountReceived = parseEther(`${500 * (i + 1)}`); // 500 or 1000 tokens
            ...
            // Check total tokens
            const totalTokens = await presale.read.getUserTotalTokens([user.account?.address!]);
            expect(totalTokens).to.equal(parseEther("1500"));
        });
        ...
        it("should return empty array for users with no purchases", async () => {
            ...
            // Check zero total tokens
            const totalTokens = await presale.read.getUserTotalTokens([user.account?.address!]);
            expect(totalTokens).to.equal(0n);
        });
    });
}); 